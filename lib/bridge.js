...
class ComfoAirQBridge extends events {
  ...
  initSocket() {
    ...
    this.sock.on('connect', () => {
      this._settings.logger('bridge : connected to comfoAir unit -> ' + (new Date(Date.now())).toLocaleString());

      this.isconnected = true;
    });

    this.sock.on('timeout', () => {
      this._settings.logger('bridge : TCP socket timeout -> ' + (new Date(Date.now())).toLocaleString());
      const reason = {
        error: 'timeout'
      };
      if (this.isconnected) {
    ...
    this.sock.on('data', (data) => {
      ...
      while (offset < datalen) {
        ...
        if (this._settings.debug) {
          this._settings.logger(' <- RX : ' + buffer.toString('hex'));
        }
    ...
    this.sock.on('close', (had_error) => {
      if (had_error) {
        this._settings.logger('bridge : TCP socket closed with error -> ' + (new Date(Date.now())).toLocaleString());
      } else {
        this._settings.logger('bridge : TCP socket closed -> ' + (new Date(Date.now())).toLocaleString());
      }
    ...
    this.sock.on('end', () => {
      this._settings.logger('bridge : TCP socket ended -> ' + (new Date(Date.now())).toLocaleString());
  ...
  async discovery() {
    ...
    return new Promise((resolve, reject) => {
      listener.bind(this._settings.port, () => {
        ...
        if (this._settings.debug) {
          this._settings.logger(' -> TX (UDP) : ' + txdata.toString('hex'));
        }
      ...
      listener.on('message', (message, remote) => {
        if (this._settings.debug) {
          this._settings.logger(' <- RX (UDP) : ' + message.toString('hex'));
          this._settings.logger('         (' + remote.address + ':' + remote.port + ')');
        }
        ...
        this.discoveredDevice = {
          ...
          'comfoUuid': protoData.searchGatewayResponse.uuid.toString('hex'),
          'version': protoData.searchGatewayResponse.version
        };
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      this.discovery()
        .then((result) => {
          if (this._settings.debug) {
            this._settings.logger('bridge : discovery complete -> ' + (new Date(Date.now())).toLocaleString());
          }
          resolve(result);
        });
...
ComfoAirQBridge.prototype.transmit = async function(data) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (this._settings.debug) {
      this._settings.logger(' -> TX : ' + txdata.toString('hex'));
    }

    this.sock.write(txdata, (err) => {
      if (err) {
        if (this._settings.debug) {
            this._settings.logger('bridge : error sending data -> ' + err + ' -> ' + (new Date(Date.now())).toLocaleString());
        }
        reject(err);
      }
