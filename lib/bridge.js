...
class ComfoAirQBridge extends events {
  ...
  constructor (options) {
    ...
    this.sock.on('connect', () => {
      console.log('bridge : connected to comfoAir unit -> ' + (new Date(Date.now())).toISOString());

      this.isconnected = true;
    })

    this.sock.on('timeout', () => {
      console.log('bridge : TCP socket timeout -> ' + (new Date(Date.now())).toISOString());
      let reason = {
        error: 'timeout'
      }
    ...
    this.sock.on('error', (err) => {
      console.error('bridge : sock error: ' + err + ' -> ' + (new Date(Date.now())).toISOString());
      let reason = {
        error: err
      }
    ...
    this.sock.on('close', (had_error) => {
      if (had_error) {
        console.log("bridge : TCP socket closed with error -> " + (new Date(Date.now())).toISOString());
      } else {
        console.log("bridge : TCP socket closed -> " + (new Date(Date.now())).toISOString());
      }
    ...
    this.sock.on('end', () => {
      console.log("bridge : TCP socket ended -> " + (new Date(Date.now())).toISOString());
  ...
  return new Promise((resolve,reject) => {
    ...
    try {
      this.discovery()
        .then((result) => {
          if (this.debug) {
            console.log('  discovery complete -> ' + (new Date(Date.now())).toISOString())
          }
...
ComfoAirQBridge.prototype.transmit = async function (data) {
  ...
  return new Promise((resolve, reject) => {
    ...
    this.sock.write(txdata, (err) => {
      if (err) {
        console.log("bridge : error sending data -> " + err + " -> " + (new Date(Date.now())).toISOString());
        reject(err);
      }
