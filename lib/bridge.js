  ...
  constructor (options) {
    ...
    this.sock.on('data', (data) => {
      ...
      while (offset < datalen) {
        ...
        //console.log("** len : " + datalen.toString().padStart(3, '0') + " - msg : " + msglen.toString().padStart(3, '0') + " - off : " + offset.toString().padStart(3, '0'));
...
async function sendBuffer(options, data) {
//  return new Promise(async (resolve, reject) => {
  if (bridge == null) {
    bridge = new zehnderBridge(options);

    let result = await bridge.discovery();
  }

  try {

    let op_len = data.operation.length; //  buffer.length - 2;
