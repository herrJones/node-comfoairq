...
class ComfoAirQ extends events {
  constructor(options) {
    ...
    this._bridge.on('disconnect', () => {
      ...
      debug('comfo: DISCONNECTED -> ' + getTimestamp());
      this._status.connected = false;
...
ComfoAirQ.prototype._keepalive = async function () {
  ...
  if (this.rxlist.length > 0) {
    this.rxlist.forEach((element) => {
      ...
      if (diff.valueOf() > this._settings.keepalive) {
        console.log('timout receiving: ' + JSON.stringify(element) + ' -> ' + getTimestamp());
      }
    });
  }
  ...
  this.KeepAlive()
    .then(() => {
      ...
      console.log('error sending KeepAlive: ' + reason + ' -> ' + getTimestamp());
    });
...
ComfoAirQ.prototype._reconnect = function () {
  ...
  console.log('** starting reconnection -> ' + getTimestamp()());
    ...
    try {
      ...
      this._bridge.transmit(txData)
        .then(async () => {
          ...
          console.log('comfo : TX reject -> ' + reason + ' -> ' + getTimestamp()());
          reject(reason);
        });
      
    }
    catch (exc) {
      console.log('comfo : TX error -> ' + JSON.stringify(exc) + ' -> ' + getTimestamp());
    ...
    try {
      ...
      this._bridge.transmit(txData)
        .then(() => {
          ...
          this.sensors = [];
          console.log('comfo : session closed -> ' + getTimestamp());
          resolve({});
        },(reason) => {
          console.log('comfo : TX reject -> ' + reason + ' -> ' + getTimestamp());
          reject(reason);
