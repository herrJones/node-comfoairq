...
const comfoIP = '10.0.0.14';
const comfoPin = 4321;

const localUUID = Buffer.from('20200428000000000000000009080407', 'hex');
const comfoDevice = 'node-comfoairq';
  ...
  constructor() {
    ...
    this._bridge.on('received', (data) => {
      ...
      if (this.rxlist.length > 0) {
        const idx = this.rxlist.findIndex(({
          kind
        }) => kind === data.kind);
        if (idx >= 0) {
